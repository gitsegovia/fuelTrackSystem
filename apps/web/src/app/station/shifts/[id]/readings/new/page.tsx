...
export default function NewReadingsPage() {
  ...
  // Todas las boquillas operativas de la estación
  ...
  // Solo las que aún no tienen lectura de este tipo
  const pendingNozzles = nozzles.filter((n) => !recordedForType.has(n.id))
  // Las que ya tienen lectura de este tipo (solo para mostrar)
    ...
    return (
      <div className="max-w-lg space-y-4">
        <PageHeader title={title} description={subtitle}
          action={<Button variant="outline" size="sm" onClick={() => router.back()}><ArrowLeft className="size-4" /> Volver</Button>}
        />
  ...
  if (pendingNozzles.length === 0) {
    return (
      <div className="max-w-lg space-y-4">
        ...
        <Button variant="outline" onClick={() => router.back()}>Volver al turno</Button>
      </div>
    )
  }
  ...
  return (
    <div className="max-w-lg space-y-6">
      ...
      {/* Ya registradas (solo informativo) */}
