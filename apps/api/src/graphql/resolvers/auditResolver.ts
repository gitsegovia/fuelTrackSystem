...
function buildDateWhere(field: string, startDate?: Date, endDate?: Date) {
...
function round(v: number, dec = 4): number {
...
const auditResolver: IResolvers<Context> = {
  Query: {
    // 1. Auditoría de Recepciones (Factura vs Descarga)
    invoiceAudit: async (
      _,
      ...
      const invoices = await ctx.models.Invoice.findAll({
        ...
        include: [
          ...
          {
            model: ctx.models.DispatchReception,
    ...
    driverAuditSummary: async (
      ...
      for (const inv of invoices as any[]) {
        ...
        const invoicedLiters = toNum(inv.liters);
        const receivedLiters = (inv.dispatchReceptions ?? []).reduce(
          (s: number, dr: any) => s + toNum(dr.receivedLiters),
          0
        );
      ...
      return Array.from(driverMap.values()).map((d) => ({
        ...
        avgDifferentialPercent: d.totalInvoicedLiters > 0
          ? round((d.totalDifferential / d.totalInvoicedLiters) * 100)
          : 0,
      })).sort((a, b) => a.totalDifferential - b.totalDifferential);
    },

    // 3. Triple diferencial por turno
    shiftAudit: async (
      _,
      { gasStationId, startDate, endDate }: { gasStationId: string; startDate?: Date; endDate?: Date },
      ctx: Context
      ...
      return Promise.all(
        (shifts as any[]).map((shift) => resolveShiftAuditRow(shift, ctx))
      );
    },
    ...
    shiftAuditDetail: async (_, { shiftId }: { shiftId: string }, ctx: Context) => {
      ...
      return resolveShiftAuditRow(shift as any, ctx);
    },
    ...
    dispatcherAudit: async (
      ...
      return Array.from(empMap.values()).map((e) => ({
        ...
        avgDifferentialPerTicket: e.totalTickets > 0
          ? round(e.totalDifferential / e.totalTickets)
          : 0,
        shortTicketPercent: e.totalTickets > 0
          ? round((e.shortTickets / e.totalTickets) * 100)
          : 0,
      })).sort((a, b) => a.totalDifferential - b.totalDifferential);
    },
    ...
    tankBalanceAudit: async (
      ...
      return resolveTankBalance(tankId, startDate, endDate, ctx);
    },
    ...
    tankBalanceAuditByStation: async (
      ...
      return Promise.all(
        (tanks as any[]).map((tank) => resolveTankBalance(tank.id, startDate, endDate, ctx, tank))
      );
    },
...
// ─── Helper: calcular fila de auditoría de turno ──────────────────────────────

async function resolveShiftAuditRow(shift: any, ctx: Context) {
  const shiftId = shift.id;

  // A) Medidor: lecturas INITIAL y FINAL por boquilla en este turno
  ...
  if (hasValidReadings && meterDeltaLiters !== null) {
    meterDeltaLiters = round(meterDeltaLiters);
  }

  // B) Cajero: tickets del turno
  const tickets = await ctx.models.SalesTicket.findAll({
  ...
  for (const t of tickets as any[]) {
    totalTickets++;
    if (t.status === SalesTicketStatus.CANCELED) {
      canceledTickets++;
      continue;
    }
  ...
  // C) Tanque: medición SHIFT_CLOSURE ligada a este turno
  const tankMeasurement = await ctx.models.TankMeasurement.findOne({
    where: {
      employeeShiftId: shiftId,
      measurementReason: MeasurementReason.SHIFT_CLOSURE,
    },
...
// ─── Helper: balance de tanque ────────────────────────────────────────────────

async function resolveTankBalance(
  tankId: string,
  startDate: Date,
  endDate: Date,
  ctx: Context,
  ...
  // Medición de apertura: la más reciente antes o igual a startDate
  const openingMeasurement = await ctx.models.TankMeasurement.findOne({
    where: { tankId, measurementTime: { [Op.lte]: startDate } },
    order: [["measurementTime", "DESC"]],
  });

  // Medición de cierre: la más reciente hasta endDate
  ...
  // Despachos en el período: SalesTickets → via boquillas → dispensadores → tanque
  ...
  const variancePercent = expectedClosingVolume !== 0
    ? round((varianceLiters / expectedClosingVolume) * 100)
    : 0;
  ...
  return {
    ...
    actualClosingVolume: round(actualClosingVolume),
    varianceLiters,
    variancePercent,
  };
}
